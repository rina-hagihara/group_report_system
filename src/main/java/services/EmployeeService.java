...
public class EmployeeService extends ServiceBase {
    ...
    public List<MissionEmployeeView> getAssignedMission(EmployeeView ev){
        Employee e = EmployeeConverter.toModel(ev);
        List<MissionEmployee> me = em.createNamedQuery(JpaConst.Q_ME_GET_ALL_MINE)
                                    .setParameter(JpaConst.JPQL_PARM_EMPLOYEE, e)
                                    .getResultList();
        List<MissionEmployeeView> mes = MissionEmployeeConverter.toViewList(me);
        return mes;
    }
