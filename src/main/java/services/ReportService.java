...
public class ReportService extends ServiceBase {
     ...
     */
    public List<String> create(MissionService service, MissionReportView mrv) {
        ReportView rv = mrv.getReport();
        List<String> errors = ReportValidator.validate(rv);
        if (errors.size() == 0) {
            LocalDateTime ldt = LocalDateTime.now();
            rv.setCreatedAt(ldt);
            rv.setUpdatedAt(ldt);
            createInternal(service, mrv);
        }
     ...
     * @param rv 日報データ
     */
    private void createInternal(MissionService service, MissionReportView mrv) {

        em.getTransaction().begin();
        //日報のpersistを行う
        MissionReport mr = MissionReportConverter.toModel(mrv);
        Report r = mr.getReport();
        em.persist(r);
        em.persist(mr);
    ...
    /**
     * 日報案件から「案件情報」をもとに得た日報をページ数に応じて表示
     * @param m
     * @param page
     * @return
     */
    public List<MissionReport> getReportByMission(Mission m, int page){
        List<MissionReport> missions = em.createNamedQuery(JpaConst.Q_MR_GET_REPORTS_BY_MIS, MissionReport.class)
